...
def nq(phi_deg: float) -> float:
    phi = math.radians(phi_deg)
    return math.exp(math.pi * math.tan(phi)) * math.tan(math.pi/4 + phi/2)**2

def area_mobilised(z_m: float, geom: Geometry, exponent: float = 3.0) -> float:
    h = max(geom.tip_or_skirt_depth_m, 0.1)
    ratio = min(max(z_m / h, 0.0), 1.0)
    return geom.max_area_m2 * ratio**exponent

def ks(z_m: float, D: float, a_s: float = 0.05) -> float:
    z_star = z_m / D
    return z_star / (z_star + a_s) if z_m > 0 else 0.0

def kc(z_m: float, D: float, alpha_c: float = 3.0) -> float:
    return 1.0 + alpha_c * (z_m / D)

def su_at_depth(z_m: float, layer: SoilLayer) -> float:
...
def base_sand(z_m: float, geom: Geometry, layer: SoilLayer) -> float:
...
def base_clay(z_m: float, geom: Geometry, layer: SoilLayer, nc: float = 6.0) -> float:
    return area_mobilised(z_m, geom) * nc * su_at_depth(z_m, layer) * kc(z_m, geom.diameter_m) / 1000.0

def skirt_resistance(z_m: float, geom: Geometry, layer: SoilLayer, alpha_s: float = 0.22, delta_frac: float = 0.08) -> float:
...
def hanna_meyerhof_branch(z_m: float, geom: Geometry, top_layer: SoilLayer, lower_layer: SoilLayer, beta: float = 0.45) -> float:
...
def layered_response(z_m: float, geom: Geometry, top_layer: SoilLayer, lower_layer: SoilLayer) -> Tuple[float, str]:
...
def total_resistance(z_m: float, geom: Geometry, soil_class: SoilClass, layers: List[SoilLayer]) -> Tuple[float, str]:
...
def solve_curve(geom: Geometry, loads: Loads, soil_class: SoilClass, layers: List[SoilLayer], z_max_m: float = 12.0, dz_m: float = 0.001):
    z_vals, q_vals = [], []
    z_sw = None
    z_pl = None
    mechanism = "unknown"
